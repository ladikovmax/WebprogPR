...
// Панель наукових функцій
const sciPanel = document.createElement('div');
sciPanel.style.display = 'flex';
sciPanel.style.overflowX = 'auto';
sciPanel.style.padding = '10px 20px';
sciPanel.style.gap = '15px';
sciPanel.style.borderBottom = '1px solid #1c1c1c';
sciPanel.style.flexShrink = '0'; // Заборона стискання панелі
sciPanel.style.scrollbarWidth = 'none';

const sciFunctions = ['sin', 'cos', 'tan', 'log', 'ln', '√', 'π', 'e', '(', ')'];
    ...
    if (value === '⌫') {
        ...
        for (const func of sciFuncs) {
            if (currentExpression.endsWith(func)) {
                // Видаляємо кількість символів, що дорівнює довжині знайденої функції
...
function evaluateExpression() {
    ...
    let mathString = currentExpression
        ...
        .replace(/π/g, 'Math.PI')
        .replace(/e/g, 'Math.E')
        .replace(/%/g, '/100') // Додано обробку відсотків (5% перетворюється на 5/100)
